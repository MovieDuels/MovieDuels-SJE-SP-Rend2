...
extern vmCvar_t cg_SFXSabersGlowSizeCW;
extern vmCvar_t cg_SFXSabersCoreSizeCW;
extern vmCvar_t cg_SFXSabersGlowSizeMaul;
extern vmCvar_t cg_SFXSabersCoreSizeMaul;
	...
	*shadow_plane = 0;
		...
		//If it has an animOffset it's a cinematic anim
	{
		...
		gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel, cent->gent->rootBone,
			&bolt_matrix, tempAngles, cent->lerpOrigin,
	...
	if (cent->gent->client->NPC_class == CLASS_ATST)
	{
		...
		gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel, cent->gent->footRBolt,
			&bolt_matrix, tempAngles, cent->lerpOrigin, cg.time,
			cgs.model_draw, cent->currentState.modelScale);
		gi.G2API_GiveMeVectorFromMatrix(bolt_matrix, ORIGIN, side_origin);
		side_origin[2] += 30; //fudge up a bit for coplaner
...
static void CG_PlayerFootsteps(const centity_t* cent, const footstepType_t foot_step_type)
{
	...
	{
		mdxaBone_t bolt_matrix;
		vec3_t tempAngles{}, side_origin, foot_down_dir;
...
static void CG_ForceElectrocution(const centity_t* cent, const vec3_t origin, vec3_t tempAngles,
...
static void CG_DoRebelsSaber(vec3_t origin, vec3_t dir, float length, float length_max, float radius, saber_colors_t color,
	int rfx, qboolean do_light)
{
	vec3_t mid, dif;
	qhandle_t blade = 0, glow = 0;
	...
	{
	case SABER_RED:
		glow = cgs.media.redRebelsGlowShader;
		blade = cgs.media.rebelsredSaberCoreShader;
		ignite = cgs.media.redIgniteFlare;
		break;
	case SABER_ORANGE:
		glow = cgs.media.orangeRebelsGlowShader;
		blade = cgs.media.rebelsorangeSaberCoreShader;
		ignite = cgs.media.orangeIgniteFlare;
		break;
	case SABER_YELLOW:
		glow = cgs.media.yellowRebelsGlowShader;
		blade = cgs.media.rebelsyellowSaberCoreShader;
		ignite = cgs.media.yellowIgniteFlare;
		break;
	case SABER_GREEN:
		glow = cgs.media.greenRebelsGlowShader;
		blade = cgs.media.rebelsgreenSaberCoreShader;
		ignite = cgs.media.greenIgniteFlare;
		break;
	case SABER_BLUE:
		glow = cgs.media.blueRebelsGlowShader;
		blade = cgs.media.rebelsblueSaberCoreShader;
		ignite = cgs.media.blueIgniteFlare;
		break;
	case SABER_PURPLE:
		glow = cgs.media.purpleRebelsGlowShader;
		blade = cgs.media.rebelspurpleSaberCoreShader;
		ignite = cgs.media.purpleIgniteFlare;
		break;
	case SABER_WHITE:
		glow = cgs.media.rgbRebelsGlowShader;
	...
	effectradius = (radius * 1.6f * v1 + Q_flrand(-1.0f, 1.0f) * 0.1f) * radiusmult * cg_SFXSabersGlowSizeRebels.value;

	coreradius = (radius * 0.4f * v2 + Q_flrand(-1.0f, 1.0f) * 0.1f) * radiusmult * cg_SFXSabersCoreSizeRebels.value;
...
static void CG_DoCWSaber(vec3_t origin, vec3_t dir, float length, float length_max, float radius, saber_colors_t color,
	int rfx, qboolean do_light)
{
	vec3_t mid, dif;
	qhandle_t blade = 0, glow = 0;
	...
	{
	case SABER_RED:
		glow = cgs.media.redCWGlowShader;
		blade = cgs.media.cwredSaberCoreShader;
		ignite = cgs.media.redIgniteFlare;
		break;
	case SABER_ORANGE:
		glow = cgs.media.orangeCWGlowShader;
		blade = cgs.media.cworangeSaberCoreShader;
		ignite = cgs.media.orangeIgniteFlare;
		break;
	case SABER_YELLOW:
		glow = cgs.media.yellowCWGlowShader;
		blade = cgs.media.cwyellowSaberCoreShader;
		ignite = cgs.media.yellowIgniteFlare;
		break;
	case SABER_GREEN:
		glow = cgs.media.greenCWGlowShader;
		blade = cgs.media.cwgreenSaberCoreShader;
		ignite = cgs.media.greenIgniteFlare;
		break;
	case SABER_BLUE:
		glow = cgs.media.blueCWGlowShader;
		blade = cgs.media.cwblueSaberCoreShader;
		ignite = cgs.media.blueIgniteFlare;
		break;
	case SABER_PURPLE:
		glow = cgs.media.purpleCWGlowShader;
		blade = cgs.media.cwpurpleSaberCoreShader;
		ignite = cgs.media.purpleIgniteFlare;
		break;
	case SABER_WHITE:
		glow = cgs.media.rgbCWGlowShader;
		blade = cgs.media.cwSaberCoreShader;
		ignite = cgs.media.whiteIgniteFlare;
		break;
	case SABER_RGB:
		glow = cgs.media.rgbCWGlowShader;
	...
	case SABER_BLACK:
		glow = cgs.media.blackSaberGlowShader;
		blade = cgs.media.cwblackSaberCoreShader;
		ignite = cgs.media.blackIgniteFlare;
		break;
	case SABER_UNSTABLE_RED:
		glow = cgs.media.redSaberGlowShader;
	...
	effectradius = (radius * 1.6f * v1 + Q_flrand(-1.0f, 1.0f) * 0.1f) * radiusmult * cg_SFXSabersGlowSizeCW.value;

	coreradius = (radius * 0.4f * v2 + Q_flrand(-1.0f, 1.0f) * 0.1f) * radiusmult * cg_SFXSabersCoreSizeCW.value;
...
static void CG_DoMaulSaber(vec3_t origin, vec3_t dir, float length, float length_max, float radius, saber_colors_t color,
	int rfx, qboolean do_light)
{
	vec3_t mid, dif;
	qhandle_t blade = 0, glow = 0;
	...
	{
	case SABER_RED:
		glow = cgs.media.redMaulGlowShader;
		blade = cgs.media.maulredSaberCoreShader;
		ignite = cgs.media.redIgniteFlare;
		break;
	case SABER_ORANGE:
		glow = cgs.media.orangeMaulGlowShader;
		blade = cgs.media.maulorangeSaberCoreShader;
		ignite = cgs.media.orangeIgniteFlare;
		break;
	case SABER_YELLOW:
		glow = cgs.media.yellowMaulGlowShader;
		blade = cgs.media.maulyellowSaberCoreShader;
		ignite = cgs.media.yellowIgniteFlare;
		break;
	case SABER_GREEN:
		glow = cgs.media.greenMaulGlowShader;
		blade = cgs.media.maulgreenSaberCoreShader;
		ignite = cgs.media.greenIgniteFlare;
		break;
	case SABER_BLUE:
		glow = cgs.media.blueMaulGlowShader;
		blade = cgs.media.maulblueSaberCoreShader;
		ignite = cgs.media.blueIgniteFlare;
		break;
	case SABER_PURPLE:
		glow = cgs.media.purpleMaulGlowShader;
		blade = cgs.media.maulpurpleSaberCoreShader;
		ignite = cgs.media.purpleIgniteFlare;
		break;
	case SABER_WHITE:
		glow = cgs.media.rgbMaulGlowShader;
		blade = cgs.media.maulSaberCoreShader;
		ignite = cgs.media.whiteIgniteFlare;
		break;
	case SABER_RGB:
		glow = cgs.media.rgbMaulGlowShader;
	...
	case SABER_BLACK:
		glow = cgs.media.blackSaberGlowShader;
		blade = cgs.media.maulblackSaberCoreShader;
		ignite = cgs.media.blackIgniteFlare;
		break;
	case SABER_UNSTABLE_RED:
		glow = cgs.media.redSaberGlowShader;
	...
	effectradius = (radius * 1.6f * v1 + Q_flrand(-1.0f, 1.0f) * 0.1f) * radiusmult * cg_SFXSabersGlowSizeMaul.value;

	coreradius = (radius * 0.4f * v2 + Q_flrand(-1.0f, 1.0f) * 0.1f) * radiusmult * cg_SFXSabersCoreSizeMaul.value;
	...
	*/
			...
			//was 2 because it would go through architecture and leave saber trails on either side of the brush - but still looks bad if we hit a corner, blade is still 8 longer than hit
		{
			...
			if (trace.fraction < 1.0f)
			{
				...
				else
				{
					if (!no_marks)
					{
						const saberInfo_t* saber = &client->ps.saber[saberNum];
						const bladeInfo_t* blade = &saber->blade[bladeNum];

						// Convert bool → qboolean explicitly
						const qboolean usingSecondStyle =
							static_cast<qboolean>(WP_SaberBladeUseSecondBladeStyle(saber, bladeNum));
	...
	if (cg_SFXSabers.integer == 0 || cg_SFXSabers.integer == 9 || cg_SFXSabers.integer == 10 || cg_SFXSabers.integer == 11)
	{
					...
					// if we have a stale segment, don't draw until we have a fresh one
				{
							...
							client->ps.saber[saberNum].trailStyle2 != 1))
					{
						...
						case SABER_GREEN:
							...
							break;
						case SABER_BLUE:
							if (cg_SFXSabers.integer == 9 || cg_SFXSabers.integer == 10 || cg_SFXSabers.integer == 11)
							{
					...
					if (diff <= SABER_TRAIL_TIME * 2)
					{
						...
						{
							fx->mShader = cgs.media.rebelsSaberBlurShader;
							duration = saber_trail->duration / (PM_InKataAnim(cg.snap->ps.torsoAnim) ? 20.0f : 10.0f);
						}
						else if (cg_SFXSabers.integer == 10)
						{
							fx->mShader = cgs.media.cwSaberBlurShader;
							duration = saber_trail->duration / (PM_InKataAnim(cg.snap->ps.torsoAnim) ? 20.0f : 10.0f);
						}
						else if (cg_SFXSabers.integer == 11)
						{
							fx->mShader = cgs.media.maulSaberBlurShader;
							duration = saber_trail->duration / (PM_InKataAnim(cg.snap->ps.torsoAnim) ? 20.0f : 5.0f);
						}
						else
						{
		...
		else
		{
			...
			{
				CG_DoRebelsSaber(org, axis[0], length, client->ps.saber[saberNum].blade[bladeNum].lengthMax,
					client->ps.saber[saberNum].blade[bladeNum].radius,
					client->ps.saber[saberNum].blade[bladeNum].color, renderfx,
					static_cast<qboolean>(!no_dlight));
			}
			...
			{
				CG_DoCWSaber(org, axis[0], length, client->ps.saber[saberNum].blade[bladeNum].lengthMax,
					client->ps.saber[saberNum].blade[bladeNum].radius,
					client->ps.saber[saberNum].blade[bladeNum].color, renderfx,
					static_cast<qboolean>(!no_dlight));
			}
			...
			{
				CG_DoMaulSaber(org, axis[0], length, client->ps.saber[saberNum].blade[bladeNum].lengthMax,
					client->ps.saber[saberNum].blade[bladeNum].radius,
					client->ps.saber[saberNum].blade[bladeNum].color, renderfx,
					static_cast<qboolean>(!no_dlight));
			}
	...
	else
	{
				...
				case 1:
					...
					else
					{
						CG_DoEp1Saber(cent, fx->mVerts[0].origin, fx->mVerts[1].origin, fx->mVerts[2].origin,
							fx->mVerts[3].origin, client->ps.saber[saberNum].blade[bladeNum].lengthMax,
							client->ps.saber[saberNum].blade[bladeNum].radius,
							client->ps.saber[saberNum].blade[bladeNum].color, renderfx,
							static_cast<qboolean>(client->ps.saber[saberNum].numBlades < 3 &&
								!(client->ps.saber[saberNum].saberFlags2 & SFL2_NO_DLIGHT)));
					}
					break;
				case 2:
		...
		if (cg.time > saber_trail->inAction)
		{
			...
			{
				fx->mShader = cgs.media.cwSwordTrailShader;
			}
			else if (cg_SFXSabers.integer == 11)
			{
				fx->mShader = cgs.media.maulSwordTrailShader;
			}
			else
			{
...
===============
*/
extern qboolean G_GetRootSurfNameWithVariant(gentity_t* ent, const char* rootSurfName, char* returnSurfName, const int returnSize);
