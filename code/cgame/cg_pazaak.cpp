...
void CG_Pazaak_OwnerDraw(int ownerDraw, float x, float y, float w, float h, vec4_t color, qhandle_t shader, float scale) {
    ...
    cgi_R_Font_DrawString((int)(x + w/2 - 10), (int)y, buf, white, cgs.media.qhFontMedium, -1, 0.9f * scale);

    // draw pot
    Com_sprintf(buf, sizeof(buf), "Pot: %d", pzk_pot);
    cgi_R_Font_DrawString((int)(x + w/2 - 10), (int)(y + 14), buf, white, cgs.media.qhFontSmall, -1, 0.75f * scale);
    ...
    for (int p = 0; p < 2; ++p) {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (pzk_field[p][i] >= 0) {
                ...
                cgi_R_Font_DrawString((int)(sx + 2), (int)(rowY + 2), cbuf, white, cgs.media.qhFontSmall, -1, 0.6f * scale);
            }
        }
        // draw points
        Com_sprintf(buf, sizeof(buf), "P%d: %d", p, pzk_points[p]);
        cgi_R_Font_DrawString((int)x, (int)(rowY + 20), buf, white, cgs.media.qhFontSmall, -1, 0.8f * scale);
        // draw credits for this player
        Com_sprintf(buf, sizeof(buf), "Credits: %d", pzk_credits[p]);
        cgi_R_Font_DrawString((int)(x + w - 70), (int)(rowY + 20), buf, white, cgs.media.qhFontSmall, -1, 0.75f * scale);
    }
}
