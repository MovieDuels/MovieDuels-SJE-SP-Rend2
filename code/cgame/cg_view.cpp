...
static void CG_CalcIdealThirdPersonViewTarget()
{
	...
	if (cg.predictedPlayerState.pm_flags & PMF_DUCKED)
	{
		...
		CG_Trace(&trace, cameraFocusLoc, cameramins, cameramaxs, nudgepos,
			uses_view_entity ? cg.snap->ps.viewEntity : cg.predictedPlayerState.client_num, MASK_CAMERACLIP);
...
static void CG_ResetThirdPersonViewDamp()
{
	...
	CG_Trace(&trace, cameraFocusLoc, cameramins, cameramaxs, cameraCurTarget, cg.predictedPlayerState.client_num,
		MASK_CAMERACLIP);
	if (trace.fraction <= 1.0)
	{
	...
	CG_Trace(&trace, cameraCurTarget, cameramins, cameramaxs, cameraCurLoc, cg.predictedPlayerState.client_num,
		MASK_CAMERACLIP);
	if (trace.fraction <= 1.0)
	{
...
static void CG_UpdateThirdPersonTargetDamp()
{
		...
		&& cg_entities[cg.snap->ps.client_num].gent->activator)
	{
		...
		CG_Trace(&trace, cg_entities[cg_entities[cg.snap->ps.client_num].gent->activator->s.number].lerpOrigin,
			cameramins, cameramaxs, cameraCurTarget, cg.predictedPlayerState.client_num, MASK_CAMERACLIP);
	}
	else
	{
		CG_Trace(&trace, cameraFocusLoc, cameramins, cameramaxs, cameraCurTarget, cg.predictedPlayerState.client_num,
...
static void CG_UpdateThirdPersonCameraDamp()
{
	...
	//	CG_Trace(&trace, cameraFocusLoc, cameramins, cameramaxs, cameraCurLoc, cg.predictedPlayerState.client_num, MASK_CAMERACLIP);
		...
		&& cg_entities[cg.snap->ps.client_num].gent->activator)
	{
		//if being held by a sand creature, trace from his actual origin, since we could be underground or otherwise in solid once he eats us
		CG_Trace(&trace, cg_entities[cg_entities[cg.snap->ps.client_num].gent->activator->s.number].lerpOrigin,
			cameramins, cameramaxs, cameraCurLoc, cg.predictedPlayerState.client_num, MASK_CAMERACLIP);
	}
	else
	{
		CG_Trace(&trace, cameraCurTarget, cameramins, cameramaxs, cameraCurLoc, cg.predictedPlayerState.client_num,
