...
static qboolean G2_SetSurfaceOnOff(CGhoul2Info* ghlInfo, surfaceInfo_v& slist, const char* surfaceName, const int offFlags)
{
	...
	if (surf)
	{
		...
		slist[surf_index].offFlags |= offFlags & (G2SURFACEFLAG_OFF | G2SURFACEFLAG_NODESCENDANTS);
		return qtrue;
	}
	else
	{
...
int G2_IsSurfaceOff (CGhoul2Info *ghlInfo, surfaceInfo_v &slist, const char *surfaceName)
{
	...
	// first find if we already have this surface in the list
	surf = G2_FindSurface(ghlInfo, slist, surfaceName, &surf_index);
	if (surf)
	{
		// set descendants value
...
int G2_IsSurfaceRendered(const CGhoul2Info* ghlInfo, const char* surfaceName, const surfaceInfo_v& slist)
{
	...
	int surf_num = G2_IsSurfaceLegal((model_t*)ghlInfo->currentModel, surfaceName, &flags);
	if (surf_num != -1)
	{//must be legal
