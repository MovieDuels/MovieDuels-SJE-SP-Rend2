...
void PM_VelocityForSaberMove(const playerState_t* ps, vec3_t throw_dir)
{
	...
	switch (saber_moveData[ps->saberMove].startQuad)
	{
	case Q_BR:
	...
	switch (saber_moveData[ps->saberMove].endQuad)
	{
	case Q_BR:
...
qboolean PM_InAnimForSaberMove(int anim, const int saberMove)
{
	...
	if (anim >= BOTH_P1_S1_T_ && anim <= BOTH_H1_S1_BR)
	{
		...
		return static_cast<qboolean>(anim == saber_moveData[saberMove].animToUse);
	}
	return qfalse;
}
...
static void PM_TorsoAnimLightsaber()
{
		...
		pm->ps->weaponstate == WEAPON_CHARGING_ALT)
	{
		...
		if (pm->ps->weapon == WP_SABER && pm->ps->SaberLength() && (pm->ps->SaberActive() || !g_noIgniteTwirl->integer))
		{
			...
			{
				PM_SetSaberMove(saber_moveData[pm->ps->saberMove].chain_idle);
			}
			else
			{
...
void PM_TorsoAnimation()
{
	...
	else if (pm->ps->lastShotTime > level.time - 2000 && cg.renderingThirdPerson && pm_is_pistoleer() && (!(pm->ps->communicatingflags & (1u << CF_AIMINGGUN))))
	{
		weaponBusy = qtrue;
	}
	else if (pm->ps->lastShotTime > level.time - 1000 && cg.renderingThirdPerson && pm_is_pistoleer() && (pm->ps->communicatingflags & (1u << CF_AIMINGGUN)))
	{
		weaponBusy = qtrue;
	}
	else if (pm->ps->lastShotTime > level.time - 2000 && cg.renderingThirdPerson && (!(pm->ps->communicatingflags & (1u << CF_AIMINGGUN))))
	{
		weaponBusy = qtrue;
	}
...
qboolean PM_SaberInRollStab(const saber_moveName_t saberMove)
{
	switch (saberMove)
	{
	case LS_ROLL_STAB:
...
qboolean PM_SaberInLungeStab(const saber_moveName_t saberMove)
{
	switch (saberMove)
	{
	case LS_A_LUNGE:
