	...
	const int numFrames)
{
	...
	if (r_Ghoul2BlendMultiplier)
	{
		if (r_Ghoul2BlendMultiplier->value != 1.0f)
		{
			...
			{
				blend_time = ceil((float)ablend_time * r_Ghoul2BlendMultiplier->value);
			}
		}
	}

	// Invalid bone index or missing bone
	if (index < 0 || index >= (int)blist.size() || blist[index].boneNumber < 0)
	{
		Com_Printf("G2_Set_Bone_Anim_Index: Invalid bone index %d (boneNumber=%d)\n",
			index, (index < (int)blist.size() ? blist[index].boneNumber : -999));
		return qfalse;
	}

	// Validate set_frame
	if (!(set_frame == -1 ||
		(set_frame >= startFrame && set_frame < endFrame) ||
		(set_frame > endFrame && set_frame <= startFrame + 1)))
	{
		Com_Printf("G2_Set_Bone_Anim_Index: set_frame %.2f out of range (%d–%d)\n",
			set_frame, startFrame, endFrame);
		return qfalse;
	}
	...
	{
		float current_frame, frame_anim_speed;
		int frame, end_frame1, flags1;

		if (G2_Get_Bone_Anim_Index(
			blist, index, current_time,
			&current_frame, &frame, &end_frame1, &flags1, &frame_anim_speed, numFrames))
		{
				...
				else
				{
					...
					if (blist[index].blendFrame >= blist[index].endFrame)
					{
						if (blist[index].flags & BONE_ANIM_OVERRIDE_LOOP)
							...
							blist[index].blendFrame = (blist[index].endFrame > 0 ?
								blist[index].endFrame - 1 : 0);
					}

					// Clamp blendLerpFrame
					if (blist[index].blendLerpFrame >= blist[index].endFrame)
					{
						if (blist[index].flags & BONE_ANIM_OVERRIDE_LOOP)
							blist[index].blendLerpFrame = blist[index].startFrame;
						else
							blist[index].blendLerpFrame = (blist[index].endFrame > 0 ?
								blist[index].endFrame - 1 : 0);
					}
				}
		...
		{
			// No existing anim → disable blend
	...
	// Validate blend frames
	if (blist[index].blendFrame < 0 || blist[index].blendFrame >= numFrames)
	{
		Com_Printf("G2_Set_Bone_Anim_Index: blendFrame %d out of range (0–%d)\n",
	...
	{
		Com_Printf("G2_Set_Bone_Anim_Index: blendLerpFrame %d out of range (0–%d)\n",
	...
	if (set_frame != -1)
	{
		blist[index].startTime =
			current_time - ((set_frame - (float)startFrame) * 50.0f / animSpeed);
	}
	else
	{
...
qboolean G2_Set_Bone_Anim(const CGhoul2Info* ghlInfo, boneInfo_v& blist, const char* boneName, const int startFrame, const int endFrame, const int flags, const float animSpeed, const int current_time, const float set_frame, const int blend_time)
{
	...
	if (index != -1)
	{
		...
		if (set_frame != -1)
		{
			blist[index].startTime = current_time - (set_frame - static_cast<float>(startFrame)) * 50.0 / animSpeed;
		}
		else
		{
			blist[index].startTime = current_time;
...
qboolean G2_Get_Bone_Anim(const CGhoul2Info* ghlInfo, boneInfo_v& blist, const char* boneName, const int current_time, float* current_frame, int* startFrame, int* endFrame, int* flags, float* retAnimSpeed)
{
	...
	if (G2_Get_Bone_Anim_Index(blist, index, current_time, current_frame, startFrame, endFrame, flags, retAnimSpeed,
	...
	if (bone_index >= 0 && bone_index < static_cast<int>(blist.size()))
	{
		...
		if (blist[bone_index].pauseTime)
		{
				...
				&endFrame, &flags, &animSpeed, numFrames))
			{
				...
				G2_Set_Bone_Anim_Index(blist, bone_index, startFrame, endFrame, flags, animSpeed, current_time,
...
*/
static void G2_RagDollSolve(CGhoul2Info_v& ghoul2_v, int g2_index, float decay, bool limitAngles, CRagDollUpdateParams* params = nullptr);
...
static int G2_Set_Bone_Angles_Rag(const CGhoul2Info& ghoul2, boneInfo_v& blist, const char* boneName, const int flags, const float radius, const vec3_t angleMin = nullptr, const vec3_t angleMax = nullptr, const int blend_time = 500)
{
	int index = G2_Find_Bone_Rag(&ghoul2, blist, boneName);
...
- Preserves behaviour unless invalid data would crash
- Fixes startFrame/endFrame out-of-range crash
====================
*/
void G2_SetRagDoll(CGhoul2Info_v& ghoul2V, CRagDollParams* parms)
{
	...
	// ------------------------------------------------------------
	int model = -1;
	for (int i = 0; i < (int)ghoul2V.size(); i++)
	{
		if (ghoul2V[i].mModelindex != -1)
		{
			model = i;
			break;
		}
	...
	const int index = G2_Find_Bone_Rag(&ghoul2, blist, "model_root");

	// ------------------------------------------------------------
	// Handle ragdoll phases
	// ------------------------------------------------------------
	...
	case CRagDollParams::RP_END_DEATH_ANIM:
		...
		if (broadsword_waitforshot && broadsword_waitforshot->integer)
		{
			...
			{
				if (!(ghoul2.mFlags &
					(GHOUL2_RAG_COLLISION_DURING_DEATH | GHOUL2_RAG_COLLISION_SLIDE)))
				{
					return;
				}
			}
	...
	case CRagDollParams::RP_CORPSE_SHOT:
		// (Corpse shot logic unchanged, no asserts)
		break;
	...
	case CRagDollParams::RP_SET_PELVIS_OFFSET:
		...
		{
			boneInfo_t& bone = blist[index];
			if (bone.boneNumber >= 0 && (bone.flags & BONE_ANGLES_RAGDOLL))
			{
	...
	// ------------------------------------------------------------
	// Build skeleton and mark rag bones
	// ------------------------------------------------------------
	G2_GenerateWorldMatrix(parms->angles, parms->position);
	G2_ConstructGhoulSkeleton(ghoul2V, curTime, qfalse, parms->scale);

	// (Bone marking calls unchanged)
	...
	// ------------------------------------------------------------
	// Validate and clamp startFrame / endFrame
	// ------------------------------------------------------------
	int startFrame = parms->startFrame;
	int endFrame = parms->endFrame;
	...
	if (endFrame <= startFrame)
	{
		...
		{
			// Degenerate case: collapse to last frame
			startFrame = mod_a->numFrames - 1;
			endFrame = mod_a->numFrames - 1;
		}
	}
	...
	// ------------------------------------------------------------
	// Apply freeze anim segment if valid
	// ------------------------------------------------------------
	if (startFrame != endFrame)
	{
		G2_Set_Bone_Anim_No_BS(ghoul2, mod_a, blist, "upper_lumbar",
			startFrame, endFrame - 1,
			BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
			1.0f);

		G2_Set_Bone_Anim_No_BS(ghoul2, mod_a, blist, "lower_lumbar",
			startFrame, endFrame - 1,
			BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
			1.0f);

		G2_Set_Bone_Anim_No_BS(ghoul2, mod_a, blist, "Motion",
			startFrame, endFrame - 1,
			BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
			1.0f);

		G2_Set_Bone_Anim_No_BS(ghoul2, mod_a, blist, "lfemurYZ",
			startFrame, endFrame - 1,
			BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
			1.0f);

		G2_Set_Bone_Anim_No_BS(ghoul2, mod_a, blist, "rfemurYZ",
			startFrame, endFrame - 1,
			BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
			1.0f);

		G2_Set_Bone_Anim_No_BS(ghoul2, mod_a, blist, "rhumerus",
			startFrame, endFrame - 1,
			BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
			1.0f);

		G2_Set_Bone_Anim_No_BS(ghoul2, mod_a, blist, "lhumerus",
			startFrame, endFrame - 1,
			BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
			1.0f);
	}
	...
	// ------------------------------------------------------------
	// Continue ragdoll setup
	// ------------------------------------------------------------
	G2_ConstructGhoulSkeleton(ghoul2V, curTime, qfalse, parms->scale);

	// (PCJ / effector setup unchanged — no asserts)
	...
	G2_RagDollCurrentPosition(ghoul2V, model, curTime,
		parms->angles, parms->position, parms->scale);
	...
	for (int k = 0; k < 20; k++)
	{
		G2_RagDollSettlePositionNumeroTrois(ghoul2V, &fparms, curTime);

		G2_RagDollCurrentPosition(ghoul2V, model, curTime,
			parms->angles, dPos, parms->scale);

		G2_RagDollMatchPosition();
		G2_RagDollSolve(ghoul2V, model,
			1.0f * (1.0f - (float)k / 40.0f),
			qfalse);
	}
}
...
static void Rag_Trace(trace_t* results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, const int passEntityNum, const int contentmask, const EG2_Collision eG2TraceType, const int useLod)
{
#ifdef _DEBUG
...
static bool G2_ApplyRealBonePhysics(boneInfo_t& bone, const SRagEffector& e, const CRagDollUpdateParams* params, vec3_t goalSpot, const vec3_t goalBase, const vec3_t testMins, const vec3_t testMaxs, const float gravity, const float mass, const float bounce)
{
	trace_t tr;
	...
	if (goalBase)
	{
		VectorCopy(goalBase, used_origin);
	}
	else
	{
	...
	if (gravity)
	{
		...
		Rag_Trace(&tr, used_origin, testMins, testMaxs, ground, params->me, RAG_MASK, G2_NOCOLLIDE, 0);
	...
	Rag_Trace(&tr, used_origin, testMins, testMaxs, projected_origin, params->me, RAG_MASK, G2_NOCOLLIDE, 0);
...
static bool G2_RagDollSettlePositionNumeroTrois(CGhoul2Info_v& ghoul2_v, CRagDollUpdateParams* params, const int curTime)
{
	...
	for (i = 0; i < numRags; i++)
	{
		...
		if (has_daddy)
		{
			Rag_Trace(&tr, e.currentOrigin, testMins, testMaxs, parent_origin, ignore_num, RAG_MASK, G2_NOCOLLIDE, 0);
			//Rag_Trace(&tr, parentOrigin, testMins, testMaxs, e.currentOrigin, ignoreNum, RAG_MASK, G2_NOCOLLIDE, 0);
		}
		else
		{
			Rag_Trace(&tr, e.currentOrigin, testMins, testMaxs, params->position, ignore_num, RAG_MASK, G2_NOCOLLIDE, 0);
		}
		...
		if (tr.startsolid || tr.allsolid || tr.fraction != 1.0f)
		{
			...
			else
			{
				...
				VectorMA(params->position, 40.0f, v_sub, goalSpot);
		...
		if (start_solid)
		{
			...
			*/
			...
			Rag_Trace(&solid_tr, params->position, testMins, testMaxs, e.currentOrigin, ignore_num, RAG_MASK,
				G2_NOCOLLIDE, 0);
...
static void G2_RagDollSolve(CGhoul2Info_v& ghoul2_v, const int g2_index, const float decay, const bool limitAngles, CRagDollUpdateParams* params)
{
	CGhoul2Info& ghoul2 = ghoul2_v[g2_index];
...
#if 1 //constraint breaks?
				if (limitAngles && (all_solid_count < 16 || bone.RagFlags & RAG_UNSNAPPABLE))
...
static void G2_IKSolve(CGhoul2Info_v& ghoul2_v, const int g2_index, const float decay, const bool limitAngles)
{
	CGhoul2Info& ghoul2 = ghoul2_v[g2_index];
...
static void G2_DoIK(CGhoul2Info_v& ghoul2_v, const int g2_index, CRagDollUpdateParams* params)
{
	if (!params)
	{
		assert(0);
...
static int G2_Set_Bone_Angles_IK(const CGhoul2Info& ghoul2, boneInfo_v& blist, const char* boneName, const int flags, const float radius, const vec3_t angleMin = nullptr, const vec3_t angleMax = nullptr)
{
	int index = G2_Find_Bone_Rag(&ghoul2, blist, boneName);
